basicFeeScheduleDetail = {
    params: [],
    Load: function (params) {
        basicFeeScheduleDetail.params = params;


        var self = $('#basicFeeScheduleDetail');
        self.loadDropDowns(true).done(function () {

            basicFeeScheduleDetail.LoadBasicFeeSchedule();

        });
    },

    LoadBasicFeeSchedule: function () {
        $("#basicFeeScheduleDetail #pnlBFSPlanInfo").removeClass('disableAll');
        if (basicFeeScheduleDetail.params.mode == "Add") {


            $('#basicFeeScheduleDetail #pnlBFSPlanInfo').addClass("disableAll");
            basicFeeScheduleDetail.ValidateBasicFeeSchedule();

            //serialize data
            $('#frmBasicFeeScheduleDetail').data('serialize', $('#frmBasicFeeScheduleDetail').serialize());
        }
        else if (basicFeeScheduleDetail.params.mode == "Edit") {
            basicFeeScheduleDetail.FillBasicFeeSchedule(basicFeeScheduleDetail.params.BasicFeeScheduleId).done(function (response) {
                if (response.status != false) {
                    var basicFeeSchedule_detail = JSON.parse(response.BasicFeeScheduleFill_JSON);
                    var self = $("#basicFeeScheduleDetail");
                    utility.bindMyJSON(true, basicFeeSchedule_detail, false, self).done(function () {
                        ...
                        basicFeeScheduleDetail.LoadBasicFeeSchedulePlan().done(function (response) {
                            if (response.status != false) {
                                basicFeeScheduleDetail.BasicFeeSchedulePlanGridLoad(response);
    ...
    ValidateBasicFeeSchedule: function () {
        $('#frmBasicFeeScheduleDetail')
           .bootstrapValidator({
               live: 'disabled',
               ...
               e.preventDefault();
               basicFeeScheduleDetail.BasicFeeScheduleSave();
           });
    },

    BasicFeeScheduleSave: function () {
        var self = $("#basicFeeScheduleDetail");
        var myJSON = self.getMyJSON();
        var CPTCode = $('#basicFeeScheduleDetail #txtCPTCode').val();
                ...
                if (strMessage == "") {
                    Admin_CPTCode.ValidateCPTCode(CPTCode).done(function (res) {
                        if (res.length != 0) {
                            ...
                            if (basicFeeScheduleDetail.params.BasicFeeScheduleId == "-1") {
                                basicFeeScheduleDetail.SaveBasicFeeSchedule(myJSON).done(function (response) {
                                    if (response.status != false) {

                                        $("#basicFeeScheduleDetail #pnlBFSPlanInfo").removeClass('disableAll');
                                        basicFeeScheduleDetail.params.BasicFeeScheduleId = response.BasicFeeScheduleId;

                                        //Editable Grid
                                        var PanelGrid = "#basicFeeScheduleDetail #pnlBFSPlanInfo";
                                        var GridId = "#basicFeeScheduleDetail #dgvBFSPlanInfo";
                                        utility.MakeEditableGrid(PanelGrid, GridId, basicFeeScheduleDetail);

                                        Admin_BasicFeeSchedule.BasicFeeScheduleSearch(response.BasicFeeScheduleId);
                            ...
                            else if (basicFeeScheduleDetail.params.BasicFeeScheduleId != "-1" && basicFeeScheduleDetail.params.BasicFeeScheduleId != "" && basicFeeScheduleDetail.params.BasicFeeScheduleId != "0") {
                                basicFeeScheduleDetail.UpdateBasicFeeSchedule(myJSON, basicFeeScheduleDetail.params.BasicFeeScheduleId).done(function (response) {
                                    if (response.status != false) {
                                        Admin_BasicFeeSchedule.BasicFeeScheduleSearch(basicFeeScheduleDetail.params.BasicFeeScheduleId);
                                        utility.DisplayMessages(response.message, 1);
    ...
    BasicFeeScheduleValidateCode: function (obj) {
        ...
        utility.ValidateAutoComplete(obj, 'basicFeeScheduleDetail #hfCPTCode', true, 1);
    },

    BindAutoComplete: function (element) {

        var entityId = $('#basicFeeScheduleDetail #ddlBasicFeeGroup option:selected').attr('refvalue');
        var hiddenCrtl = $("#basicFeeScheduleDetail #hfCPTCode");
        utility.BindIMOAutoCompleteText(element, 'COMMON_IMO_CODE', 'GET_IMO_CPTCODE', hiddenCrtl, entityId, true, -1, "CPT", true, "basicFeeScheduleDetail", null, true);
                    ...
                    if (selectedValue == "" || selectedValue == "undefined") {
                        ...
                        basicFeeScheduleDetail.DeleteBasicFeeSchedulePlan(selectedValue).done(function (response) {
                            if (response.status != false) {
                                ...
                                utility.removePaginationFromGrid($('#' + basicFeeScheduleDetail.params.PanelID + ' #pnlBFSPlanInfo'));
    ...
    SaveBasicFeeSchedule: function (BasicFeeScheduleData) {
        var data = "BasicFeeScheduleData=" + BasicFeeScheduleData;
        // serach parameter , class name, command name of class 
        return MDVisionService.defaultService(data, "ADMIN_BASIC_FEE_SCHEDULE_DETAIL", "SAVE_BASIC_FEE_SCHEDULE");
    },

    UpdateBasicFeeSchedule: function (BasicFeeScheduleData, BasicFeeScheduleID) {
        var data = "BasicFeeScheduleData=" + BasicFeeScheduleData + "&BasicFeeScheduleID=" + BasicFeeScheduleID;
        // serach parameter , class name, command name of class 
        return MDVisionService.defaultService(data, "ADMIN_BASIC_FEE_SCHEDULE_DETAIL", "UPDATE_BASIC_FEE_SCHEDULE");
    },

    FillBasicFeeSchedule: function (BasicFeeScheduleID) {
        var data = "BasicFeeScheduleID=" + BasicFeeScheduleID;
        // serach parameter , class name, command name of class 
        return MDVisionService.defaultService(data, "ADMIN_BASIC_FEE_SCHEDULE_DETAIL", "FILL_BASIC_FEE_SCHEDULE");
    },

    UpdateBasicFeeScheduleActiveInactive: function (BasicFeeScheduleID, IsActive) {
        var data = "BasicFeeScheduleID=" + BasicFeeScheduleID + "&IsActive=" + IsActive;
        // serach parameter , class name, command name of class 
        return MDVisionService.defaultService(data, "ADMIN_BASIC_FEE_SCHEDULE_DETAIL", "UPDATE_BASIC_FEE_SCHEDULE_ACTIVE_INACTIVE");
    },
    ...
    OpenCPTCode: function () {
        ...
        params["FromAdmin"] = "0";
        params["ParentCtrl"] = "basicFeeScheduleDetail";
        params["EntityId"] = $('#basicFeeScheduleDetail #ddlBasicFeeGroup option:selected').attr('refvalue');
    ...
    UnLoad: function () {
        ...
        utility.UnLoadDialog("frmBasicFeeScheduleDetail", function () {
            UnloadActionPan();
        }, function () {
            UnloadActionPan();
        });
    ...
    ShowHistory: function () {
        var PanelID = 'basicFeeScheduleDetail';
        var ParentCtrl = 'basicFeeScheduleDetail';
        var ProfileName = 'Basic Fee Schedule';
        var DBTableName = 'BasicFeeSchedule';
        var ColumnKeyId = basicFeeScheduleDetail.params.BasicFeeScheduleId;
