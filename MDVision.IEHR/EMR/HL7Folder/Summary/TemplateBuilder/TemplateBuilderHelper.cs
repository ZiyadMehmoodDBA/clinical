    ...
    public class TemplateBuilderHelper
    {
        ...
        public string SearchQuestion(TemplateBuilderModel model, Int64 questionId, Int64 rpp, Int64 PageNumber)
        {
            try
            {
                ...
                var QuestionId = 0;// MDVUtility.ToInt64(searchedfieldsJson["hfQuestionId"]);

                var shortName = model.ShortName;
                var questionName = model.questionName;
                var activeStatus = model.isActive;

                var questionType = MDVUtility.ToInt64(model.questionType) <= 0 ? null : "1";

                obj = BLLClinicalObj.LoadClinicalQuestion(QuestionId, shortName, questionName, activeStatus, model.questionType, rpp, PageNumber);
                dsQuestion = obj.Data;
                if (obj.Data != null)
                {
        ...
        public string SaveQuestion(TemplateBuilderModel model, string files)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(model.hiddenQuestionID))
                    dr.QuestionId = MDVUtility.ToInt64(model.hiddenQuestionID);

                if (!string.IsNullOrEmpty(model.questionType))
                    dr.QuestionTypeId = MDVUtility.ToInt64(model.questionType);
                ...
                else if (questionTypeId == 3)
                {
                    ...
                    if (!string.IsNullOrEmpty(model.sentenceDropDown))
                        dr.Sentence = MDVUtility.ToStr(model.sentenceDropDown);
                    ...
                    if (obj.Data != null)
                    {
                        ...
                        if (dsQuestion.Tables[dsQuestion.ClinicalQuestions.TableName].Rows.Count > 0)
                        {
                            ...
                            var keyValues = new Dictionary<string, string>
                            {
                                    ...
                                    { "NewLine", MDVUtility.ToStr(dr[dsQuestion.ClinicalQuestions.IsNewLineColumn.ColumnName])},
                                    { "questionName", MDVUtility.ToStr(dr[dsQuestion.ClinicalQuestions.DescriptionColumn.ColumnName])},

                                    //Text Fields
                                    { "singleLine", MDVUtility.ToStr(dr[dsQuestion.ClinicalQuestions.FieldTypeColumn.ColumnName])},
                                    ...
                                    { "autoComplete", MDVUtility.ToStr(dr[dsQuestion.ClinicalQuestions.IsAutoCompleteColumn.ColumnName])},
                                    { "predeifined", MDVUtility.ToStr(dr[dsQuestion.ClinicalQuestions.SentenceTagColumn.ColumnName])},
                                    { "sentence", MDVUtility.ToStr(dr[dsQuestion.ClinicalQuestions.SentenceColumn.ColumnName])},

                                    //Radio Fields
                                    { "label1", MDVUtility.ToStr(dr[dsQuestion.ClinicalQuestions.BoolTrueDisplayColumn.ColumnName])},
                                    { "label2", MDVUtility.ToStr(dr[dsQuestion.ClinicalQuestions.BoolFalseDisplayColumn.ColumnName])},
                                    { "defaultValue", MDVUtility.ToStr(dr[dsQuestion.ClinicalQuestions.DefaultValueColumn.ColumnName])},
                                    //{ "predefined", MDVUtility.ToStr(dr[dsQuestion.ClinicalQuestions.SentenceTagColumn.ColumnName])},
                                    //{ "predefined", MDVUtility.ToStr(dr[dsQuestion.ClinicalQuestions.SentenceTagColumn.ColumnName])},
                                    { "sentenceForLabel1", MDVUtility.ToStr(dr[dsQuestion.ClinicalQuestions.SentenceTrueColumn.ColumnName])},
                                    { "sentenceForLabel2", MDVUtility.ToStr(dr[dsQuestion.ClinicalQuestions.SentenceFalseColumn.ColumnName])},

                                    //Drop Down Fields
                                    { "areaElements", CommasToNewLine},
                                    { "predefined", MDVUtility.ToStr(dr[dsQuestion.ClinicalQuestions.SentenceTagColumn.ColumnName])},
                                    { "sentenceDropDown", MDVUtility.ToStr(dr[dsQuestion.ClinicalQuestions.SentenceColumn.ColumnName])},
                                    ...
                                    //{ "prefedined", MDVUtility.ToStr(dr[dsQuestion.ClinicalQuestions.SentenceTagColumn.ColumnName])},
                                    { "sentenceFraction", MDVUtility.ToStr(dr[dsQuestion.ClinicalQuestions.SentenceColumn.ColumnName])},

                                    // Number Field
                                    { "questionNumberDefault", MDVUtility.ToStr(dr[dsQuestion.ClinicalQuestions.DefaultValueColumn.ColumnName])},
                                    { "questionNumberMinLength", MDVUtility.ToStr(dr[dsQuestion.ClinicalQuestions.MinValueColumn.ColumnName])},
                                    { "questionNumberMaxLength", MDVUtility.ToStr(dr[dsQuestion.ClinicalQuestions.MaxValueColumn.ColumnName])},
                                    { "questionNumberPrecision", MDVUtility.ToStr(dr[dsQuestion.ClinicalQuestions.NumberPrecesionColumn.ColumnName])},
                                    //{ "prefedined", MDVUtility.ToStr(dr[dsQuestion.ClinicalQuestions.SentenceTagColumn.ColumnName])},
                                    { "sentenceNumber", MDVUtility.ToStr(dr[dsQuestion.ClinicalQuestions.SentenceColumn.ColumnName])},

                                    // Date Field
                                    { "questionDateformat", MDVUtility.ToStr(dr[dsQuestion.ClinicalQuestions.DateTimeFormatColumn.ColumnName])},
                                    { "questionDateformatDefaultValue", MDVUtility.ToStr(dr[dsQuestion.ClinicalQuestions.DefaultValueColumn.ColumnName])},
                                    //{ "prefedined", MDVUtility.ToStr(dr[dsQuestion.ClinicalQuestions.SentenceTagColumn.ColumnName])},
                                    ...
                                    { "uploadFile", MDVUtility.ToStr(dr[dsQuestion.ClinicalQuestions.FilePathColumn.ColumnName])},
                                    { "prefedined", MDVUtility.ToStr(dr[dsQuestion.ClinicalQuestions.SentenceTagColumn.ColumnName])},
            ...
            try
            {
                ...
                if (dsQuestion.Tables[dsQuestion.ClinicalQuestions.TableName].Rows.Count > 0)
                {
                    ...
                    foreach (DataRow dr in dsQuestion.Tables[dsQuestion.ClinicalQuestions.TableName].Rows)
                    {
                        ...
                        Int64 QuestionType = MDVUtility.ToInt64(model.questionType);
                        ...
                        if (QuestionType == 1)
                        {
                            ...
                            if (!string.IsNullOrEmpty(model.sentenceDropDown))
                            {
                                var getTags = GetTokens(MDVUtility.ToStr(model.sentence));
                                string sentenceTags = string.Join(",", getTags.ToArray());
