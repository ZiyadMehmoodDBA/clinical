...
function acquireImage() {
    ...
    $('#pnlDocumentScan #btnDocumentScan').removeClass('disableAll');
...
function btnLoad_onclick() {
    var retain = false;
    if (DWObject.HowManyImagesInBuffer > 0 && DWObject._UIManager._UIView.aryCurrentSelectedImageIndicesInBuffer.length > 0) {
        selectedImagesArr = DWObject._UIManager._UIView.aryCurrentSelectedImageIndicesInBuffer;
        retain = true;
    }
    ...
    var OnFailure = function (errorCode, errorString) {
        if ((errorCode == "-1110" && errorString == "Unrecognized file extension.") || errorCode == "-1200")
...
//************************** Save Image***********************************
//--------------------------------------------------------------------------------------
function btnSave_onclick(isClose) {
    if (!checkIfImagesInBuffer()) {
        return;
    }
    ...
    var OnFailure = function (errorCode, errorString) {
        utility.DisplayMessages(errorString, 2);
        //checkErrorStringWithErrorCode(errorCode, errorString);
    };
    ...
    if (strimgType_save == "tif" && _chkMultiPageTIFF_save.checked) {
        ...
        switch (i) {
            ...
            case 2: bSave = DWObject.SaveAsPNG(strFilePath, DWObject.CurrentImageIndexInBuffer); break;
            case 3: bSave = DWObject.SaveAsTIFF(strFilePath, DWObject.CurrentImageIndexInBuffer); break;
    ...
    if (vAsyn == false) {
        if (bSave)
            ...
            utility.DisplayMessages(getErrorString(), 2);
        //appendMessage('<b>Save Image: </b>');
        //if (checkErrorString()) {
...
function btnUpload_onclick() {
    ...
    var OnSuccess = function (httpResponse) {
        ...
        checkErrorStringWithErrorCode(0, "Successfully Uploded.");
...
function retainSelectedImages(order) {
    DWObject.SelectedImagesCount = selectedImagesArr.length;
    if (selectedImagesArr.length > 0) {
        for (var i = 0; i < selectedImagesArr.length; i++) {
            var valeIndex = selectedImagesArr[i];
            if (valeIndex != -1) {
                DWObject.SetSelectedImageIndex(i, valeIndex);
            }
        }
    }
    updatePageInfo();
...
function btnRemoveAllImages_onclick() {
    ...
    utility.myConfirm('Are you sure you want to remove all document(s)?', function () {
        DWObject.RemoveAllImages();
        document.getElementById("DW_TotalImage").value = "0";
        document.getElementById("DW_CurrentImage").value = "";
        shoHideDocMoveBtn();
        selectedImagesArr = [];
        if (DWObject._UIManager._UIView.aryCurrentSelectedImageIndicesInBuffer) {
            DWObject._UIManager._UIView.aryCurrentSelectedImageIndicesInBuffer = [];
        }
        // resetSelectAllImgCheck();
    }, function () {
    }, 'Confirm Delete');
}
function btnViewScanDocumen_onclick() {
    if (!checkIfImagesInBuffer()) {
        return;
    }
    $("#frmDocumentScan #dwtcontrolContainer").removeClass("hide");
...
function customViewMode() {
    if (document.getElementById("DW_PreviewMode")) {
        var varNum = parseInt(document.getElementById("DW_PreviewMode").selectedIndex + 1);
        var btnCrop = document.getElementById("btnCrop");
        if (btnCrop) {
            var tmpstr = btnCrop.src;
        ...
        var w_ = parseInt($("#frmDocumentScan #dwtcontrolContainer").css("width").split("px")[0]) - 15;
        var h_ = parseInt($("#frmDocumentScan #dwtcontrolContainer").css("height").split("px")[0]) - 20;

        if (Document_Scan && Document_Scan.params["RefCtrl"] == "patTabInsurance") {
            w_ = w_ - 30;
            h_ = h_ - 30;
            $("#page-content-wrapper").children(".dwtcontrolMain").css("height", "100%");
            $("#dwtcontrolContainer").css("height", "100%");
        }
        else {
            if ($("#frmDocumentScan #asideBtn").hasClass("asideBtn-toggled"))
            { w_ = w_ < 1000 ? w_ - 25 : w_ - 20; }
        }


        DWObject.CustomViewMode(varNum, varNum, w_, h_);
        if ($("#page-content-wrapper canvas").length > 1) {
            if (Document_Scan && Document_Scan.params["RefCtrl"] != "patTabInsurance") {
                if ($("#frmDocumentScan #asideBtn").hasClass("asideBtn-toggled"))
                { w_ = w_ + 22; }
            }
            $($("#page-content-wrapper canvas")[1]).css("left", w_);
            if (parseInt($($("#page-content-wrapper canvas")[1]).attr("height")) < h_)
                $($("#page-content-wrapper canvas")[1]).attr("height", h_);
        }
...
function Dynamsoft_OnMouseClick(index) {
    //if (index) {
    //var currentVIndex = DWObject._UIManager._UIView.GetCurrentImageIndex();
    //if (!DWObject._UIManager._UIView.aryImageControls[index].bSelect) {
    //selectedImagesArr.push[index];
    //}
    //else
    //    selectedImagesArr.pop[index];
    //}
    updatePageInfo();
}
...
function shoHideDocMoveBtn() {
    if (Document_Scan.params.PanelID) {
...
function SelectUnSelectAllImgs(chkBox) {
    if (!checkIfImagesInBuffer()) {
        return;
    }
    if (!chkBox) {
        chkBox = $("#" + Document_Scan.params["PanelID"]).find('#chkSelectAllScnImgs');
    }
    if (chkBox && $(chkBox).is(':checked')) {
        ...
        selectedImagesArr = [];
        retainSelectedImages();
        var n = DWObject._UIManager._UIView;
        ...
        n.aryCurrentSelectedImageIndicesInBuffer = [];
        updatePageInfo();
    }
}
function resetSelectAllImgCheck() {
    var chkBox = $("#" + Document_Scan.params["PanelID"]).find('#chkSelectAllScnImgs');
...
function DefaultSelectionDWImgs() {
    if (!checkIfImagesInBuffer()) {
        return;
    }
    var chkBox = $("#" + Document_Scan.params["PanelID"]).find('#chkSelectAllScnImgs');
