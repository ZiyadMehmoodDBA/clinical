var iTrackTabs = null;
var DefaultMenuSelected = "MDVisionBilling";
        ...
        if (homeTab.Container) {
            ...
            LoadCommands().done(function () {
                ...
                if (innerScreen.length > 0) {
                    ...
                    $.when(SelectTab(tabName, 'false')).then(function () {
                        var BillingSelectedPage = localStorage.getItem("BillingSelectedPage");
                        if (BillingSelectedPage && BillingSelectedPage != "") {
                            setTimeout(function () {
                                SelectTab(BillingSelectedPage, 'false');
                            }, 300);
                        } else {
...
function SelectTab(tabId, IsMenu, Reload, UniqueId, TabTitle, event) {
    ...
    var hashSection = window.location.hash;
    if (tabId != "mstrTabBilling") {
        localStorage.setItem("BillingSelectedPage", tabId);
        if (hashSection != "#Billing/" + tabId) {
            window.location.hash = "#Billing/" + tabId;
        }
    }
    var selectedTabId = $("#mstrDivBilling .tab_selected button:first").attr("id");
...
function SelectApplicationTab(tabId, IsMenu, Reload, UniqueId, TabTitle, event) {
    ...
    var PatientId = localStorage.getItem("SelectedPatientId");
    if (tabId == "billTab" + tabId.replace(/billTab/gi, "")) {
...
function AddBillingTab(ElementName, IsMenu) {
    ...
    var eobmanualposting = ElementName.search('EOB');
    if (searchcharge != -1)
...
function SelectPatient(patientID, FormName) {
    ...
    if (IsPatientExist(patientID) == true) {
        ...
        $.when(setPatientBanner(patientID)).then(function () {
            ...
            params['mode'] = "Edit";
            //******************************
            if (localStorage.getItem("BillingSelectedPage") != null && localStorage.getItem("BillingSelectedPage") == "billTabOutOfOfficeVisits") {
...
function ClosePatientNew(patientID, bit) {
    ...
    TabsArray.filter(function (obj) {
        ...
        //    PatientTabs.push(obj)
        //else
        if (obj.MasterTabID == "mstrTabBilling" && obj.ContainerControlID == "OutOfOfficeVisits")
            PatientTabs.push(obj)
    });
