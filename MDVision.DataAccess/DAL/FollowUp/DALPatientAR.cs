...
namespace MDVision.DataAccess.DAL.FollowUp
{
    public class DALPatientAR
    {
        ...
        private const string PROC_PATIENT_AR_SELECT = "Billing.sp_PatientARDetailSelect";
        private const string PROC_PATIENT_AR_INSERT = "Billing.sp_PatientARDetailInsert";
        private const string PROC_PATIENT_AR_UPDATE = "Billing.sp_PatientARDetailUpdate";
        private const string PROC_PATIENT_AR_DELETE = "Billing.sp_PatientARDetailDelete";

        #endregion

        #region "Parameters"
        private const string PARM_FOLLOWUP_PAT_AR_DETAIL_ID = "@PatARDetailId";
        ...
        private void CreateParameters(IDBManager dbManager, DSFollowUp ds, Boolean IsInsert)
        {
            ...
            if (IsInsert == true)
                dbManager.AddParameters(0, PARM_FOLLOWUP_PAT_AR_DETAIL_ID, ds.FollowUpARDetail.FolUpARDtlIdColumn.ColumnName, DbType.Int64, ParamDirection.Output);
            else
                dbManager.AddParameters(0, PARM_FOLLOWUP_PAT_AR_DETAIL_ID, ds.FollowUpARDetail.FolUpARDtlIdColumn.ColumnName, DbType.Int64);
        ...
        public DSFollowUp LoadFollowUpPatientARDetail(Int64 FollowUpARDetailID, string PatientAccount, long ProviderId, long FacilityId, string ClaimNumber, DateTime? DOSFrom, DateTime? DOSTo, long groupId, long ActionId, long ReasonId, long InsurancePlanId, string LastName, string FirstName, string suspended, long Age, string ARType,string LastModified,string LastComment ,int PageNumber, int RowspPage)
        {
            ...
            try
            {
                ...
                if (FollowUpARDetailID == 0)
                    dbManager.AddParameters(0, PARM_FOLLOWUP_PAT_AR_DETAIL_ID, null);
                else
                    dbManager.AddParameters(0, PARM_FOLLOWUP_PAT_AR_DETAIL_ID, FollowUpARDetailID);

                if (PatientAccount == "")
                    dbManager.AddParameters(1, PARM_ACCOUNT_NUMBER, null);
                else
                    dbManager.AddParameters(1, PARM_ACCOUNT_NUMBER, PatientAccount);
                ...
                if (ClaimNumber == "")
                    dbManager.AddParameters(4, PARM_CLAIM_NUMBER, null);
                else
                    dbManager.AddParameters(4, PARM_CLAIM_NUMBER, ClaimNumber);
                ...
                if (groupId == 0)
                    dbManager.AddParameters(7, PARM_GROUP_ID, null);
                else
                    dbManager.AddParameters(7, PARM_GROUP_ID, groupId);

                if (ActionId == 0)
                    dbManager.AddParameters(8, PARM_ACTION_ID, null);
                else
                    dbManager.AddParameters(8, PARM_ACTION_ID, ActionId);

                if (ReasonId == 0)
                    dbManager.AddParameters(9, PARM_REASON_ID, null);
                else
                    dbManager.AddParameters(9, PARM_REASON_ID, ReasonId);

                if (InsurancePlanId == 0)
                    dbManager.AddParameters(10, PARM_INSURANCE_PLAN_ID, null);
                else
                    dbManager.AddParameters(10, PARM_INSURANCE_PLAN_ID, InsurancePlanId);

                if (LastName == "")
                    dbManager.AddParameters(11, PARM_PAT_LAST_NAME, null);
                else
                    dbManager.AddParameters(11, PARM_PAT_LAST_NAME, LastName);

                if (FirstName == "")
                    dbManager.AddParameters(12, PARM_PAT_FIRST_NAME, null);
                else
                    dbManager.AddParameters(12, PARM_PAT_FIRST_NAME, FirstName);

                if (suspended == "")
                    dbManager.AddParameters(13, PARM_SUSPENDED, null);
                else
                    dbManager.AddParameters(13, PARM_SUSPENDED, suspended);


                if (Age == 0)
                    dbManager.AddParameters(14, PARM_AGE, null);
                else
                    dbManager.AddParameters(14, PARM_AGE, Age);
                ...
                if (ClientConfiguration.DecryptFrom64(MDVSession.Current.AppUserName).ToUpper() == ClientConfiguration.DefaultUser.ToUpper())
                    dbManager.AddParameters(17, PARM_ENTITY_ID, null);
                else
                    dbManager.AddParameters(17, PARM_ENTITY_ID, MDVSession.Current.EntityId);

                dbManager.AddParameters(18, PARM_USER_ID, MDVSession.Current.AppUserId);

                dbManager.AddParameters(19, PARM_RECORD_COUNT, ds.FollowUpARDetail.RecordCountColumn.ColumnName, DbType.Int64, ParamDirection.Output);
                if(!string.IsNullOrEmpty(LastModified))
                dbManager.AddParameters(20, PARM_MODIFIED_ON, LastModified);
                else
                    dbManager.AddParameters(20, PARM_MODIFIED_ON, null);
                if(!string.IsNullOrEmpty(LastComment))
                dbManager.AddParameters(21, PARM_COMMENTS, LastComment);
                else
                    dbManager.AddParameters(21, PARM_COMMENTS, null);
                ds = (DSFollowUp)dbManager.ExecuteDataSet(CommandType.StoredProcedure, PROC_PATIENT_AR_SELECT, ds, ds.FollowUpARDetail.TableName);
                return ds;
            }
            catch (Exception ex)
            {
                MDVLogger.DALErrorLog("DALPatientAR::LoadFollowUpAR", PROC_PATIENT_AR_SELECT, ex);
                throw ex;
            }
            finally
            {
        ...
        public DSFollowUp InsertFollowUpPatientARDetail(DSFollowUp ds)
        {
            IDBManager dbManager = ClientConfiguration.GetDBManager();
            try
            {
                ...
                ds = (DSFollowUp)dbManager.InsertDataSet(CommandType.StoredProcedure, PROC_PATIENT_AR_INSERT, ds, ds.FollowUpARDetail.TableName);
                if (dtTemp != null)
                {
            ...
            {
                MDVLogger.DALErrorLog("DALPatientAR::InsertFollowUpARDetail", PROC_PATIENT_AR_INSERT, ex);
                throw ex;
            }
            finally
            {
        ...
        public DSFollowUp UpdateFollowUpPatientARDetail(DSFollowUp ds)
        {
            IDBManager dbManager = ClientConfiguration.GetDBManager();
            try
            {
                ...
                ds = (DSFollowUp)dbManager.UpdateDataSet(CommandType.StoredProcedure, PROC_PATIENT_AR_UPDATE, ds, ds.FollowUpARDetail.TableName);
            ...
            {
                MDVLogger.DALErrorLog("DALPatientAR::UpdateFollowUpARDetail", PROC_PATIENT_AR_UPDATE, ex);
                throw ex;
            }
            finally
            {
        ...
        public string DeleteFollowUpPatientARDetail(Int64 followUpARDetailID)
        {
            string returnVal = "";
            IDBManager dbManager = ClientConfiguration.GetDBManager();
            try
            {
                ...
                returnVal = dbManager.ExecuteScalar(CommandType.StoredProcedure, PROC_PATIENT_AR_DELETE).ToString();
            ...
            {
                MDVLogger.DALErrorLog("DALPatientAR::DeleteFollowUpARDetail", PROC_PATIENT_AR_DELETE, ex);
